...
- **Object Pooling:** `ObjectPool<T>` uses array-backed storage with lock-based synchronization for predictable behavior across IL2CPP runtimes (not `ConcurrentBag`, which can incur thread-local overhead). `ByteArrayPool` wraps `ArrayPool<byte>.Shared` (BCL battle-tested, no custom bucketing needed for Phase 6).
