...
1. Jitter must use `Random` seeded from `RandomNumberGenerator` — not `System.Random()` default seed.
2. Delay computation must cap at `MaxDelay` regardless of multiplier accumulation.
3. Policy must be immutable after construction — all properties are read-only.
...
3. Reconnection flow:
   ...
   - On success: fire `OnReconnected` event, reset retry counter, resume receive loop.
...
6. Send operations during reconnection:
   ...
   - Optional: configurable send buffer that queues messages during reconnection (bounded, default disabled).
...
2. Reconnection delay must be cancellable via `CancellationToken` (e.g., client disposal during reconnection).
3. The resilient client must present the same `IWebSocketClient` interface as the base client.
4. Old `WebSocketClient` instances must be fully disposed before creating new ones during reconnection.
5. Reconnection must preserve the original connection options (sub-protocols, headers, timeouts).
6. Thread safety: event callbacks must be safe to subscribe/unsubscribe from any thread.
...
7. Disposal during reconnection cancels pending delay and cleanup.
8. Reconnection preserves original connection options across reconnect cycles.
9. Send buffer (when enabled) replays queued messages after successful reconnection.
