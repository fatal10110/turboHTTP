...
6. Default port: 80 for `ws://`, 443 for `wss://`.
7. Generate and store the `Sec-WebSocket-Key` for server response validation.
...
2. URI parsing must handle standard WebSocket URIs including path, query string, and fragment.
...
3. Validate required response headers:
   - `Upgrade: websocket` (case-insensitive value comparison)
   - `Connection: Upgrade` (case-insensitive value comparison)
...
2. Response parsing must enforce a maximum header size limit (default 8KB) to prevent memory exhaustion from malicious servers.
3. Header parsing must be case-insensitive for header names per HTTP/1.1 semantics.
4. Accept key validation must use constant-time comparison to prevent timing side-channels.
5. Non-101 responses must preserve the response body (if any) for error diagnostics — bounded read (max 4KB).
6. Handshake timeout must be configurable and enforced via `CancellationToken`.
