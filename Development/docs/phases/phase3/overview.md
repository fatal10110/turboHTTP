...
39. **ALPN API compile-time guard:** `SslClientAuthenticationOptions.ApplicationProtocols` property and `SslApplicationProtocol` struct are .NET 5+ additions — they do NOT exist in .NET Standard 2.1 at compile time. Direct use would cause CS0117/CS0246 in Unity 2021.3. ALPN wiring uses reflection (`PropertyInfo.SetValue`, `Type.GetField`) to set `ApplicationProtocols` on `SslClientAuthenticationOptions` and access `SslApplicationProtocol.Http2`/`Http11` static fields. If any of these types/properties are not found, ALPN is silently skipped (safe for HTTP/1.1; Phase 3B HTTP/2 must verify availability as a gate).
40. **IPv6 Host header formatting:** `Uri.Host` returns bare IPv6 addresses without brackets (e.g., `::1`). HTTP/1.1 requires `Host: [::1]` per RFC 9112 Section 3.2. Serializer detects IPv6 via `Uri.HostNameType == UriHostNameType.IPv6` and wraps in brackets when auto-generating the Host header.
41. **Module initializer test determinism:** `[ModuleInitializer]` fires once per assembly load. After `HttpTransportFactory.Reset()` in tests, the factory is cleared and the module initializer will NOT re-fire. Tests must call `RawSocketTransport.EnsureRegistered()` explicitly in `[SetUp]` or use `HttpTransportFactory.SetForTesting(mock)`.
42. **Certificate revocation consistency:** Primary TLS path (`SslClientAuthenticationOptions`) explicitly sets `CertificateRevocationCheckMode = X509RevocationMode.NoCheck` to match the fallback path (`checkCertificateRevocation: false`). Without explicit setting, platform defaults may vary (e.g., macOS vs iOS vs Android).
43. **Content-Length validation:** Parser validates Content-Length is non-negative and parseable. Negative values or non-numeric strings throw `FormatException` (mapped to `NetworkError` by transport).
...
| ALPN API availability in Unity 2021.3 | Phase 3B (gate) | `SslApplicationProtocol` and `ApplicationProtocols` property may not exist. ALPN wiring uses reflection and silently skips if unavailable. HTTP/2 negotiation blocked without ALPN. Phase 3B must verify availability on target Unity version before implementing HTTP/2. |
