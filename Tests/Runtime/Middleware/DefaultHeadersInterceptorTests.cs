...
{
    public class DefaultHeadersInterceptorTests
    {
        [Test]
        public void AddsDefaultHeaders()        {
            Task.Run(async () =>
            {
                ...
                var middleware = new DefaultHeadersInterceptor(defaults);
        ...
        public void OverridesWhenConfigured()        {
            Task.Run(async () =>
            {
                ...
                var middleware = new DefaultHeadersInterceptor(defaults, overrideExisting: true);
                var transport = new MockTransport();
                var pipeline = new TestInterceptorPipeline(new[] { middleware }, transport);
        ...
        [Test]
        public void ClonesRequestWhenInjectingHeaders()        {
            Task.Run(async () =>
            {
                var defaults = new HttpHeaders();
                ...
                Assert.AreEqual("value", context.Request.Headers.Get("X-Added"));
