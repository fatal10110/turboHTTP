...
{
    public class MonitorInterceptorTests
    {
        [SetUp]
        public void SetUp()
        {
        ...
        {
            var middleware = new MonitorInterceptor();
        ...
        public void ClearHistoryNotifiesListenersAndEmptiesBuffer()
        {
            AssertAsync.Run(async () =>
            {
                ...
                try
                {
                    ...
                    Assert.AreEqual(1, MonitorInterceptor.HistoryCount);
                    MonitorInterceptor.ClearHistory();

                    Assert.AreEqual(0, MonitorInterceptor.HistoryCount);
        ...
        public void ListenerFailuresDoNotBlockOtherSubscribers()
        {
            AssertAsync.Run(async () =>
            {
                ...
                MonitorInterceptor.OnRequestCaptured += badHandler;
                MonitorInterceptor.OnRequestCaptured += goodHandler;
                try
                {
                    var request = new UHttpRequest(HttpMethod.GET, new Uri("https://test.com"));
                    var pipeline = new TestInterceptorPipeline(
        ...
        private static void ResetMonitor()
        {
            ...
            MonitorInterceptor.HeaderValueTransform = null;
            MonitorInterceptor.DiagnosticLogger = null;
            MonitorInterceptor.HistoryCapacity = 1000;
